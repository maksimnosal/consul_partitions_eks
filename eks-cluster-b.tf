...
resource "aws_security_group_rule" "eks_to_nodes_cluster-b" {
  ...
  type = "ingress"
  source_security_group_id = module.eks-cluster-b.cluster_security_group_id
  from_port = 1025
  to_port = 10000
  security_group_id = module.eks-cluster-b.node_security_group_id
  protocol = "tcp"
}
...
resource "aws_security_group_rule" "cluster-a_nodes_to_cluster-b_eks" {
  ...
  from_port = 443
  to_port = 443
  security_group_id = module.eks-cluster-b.cluster_security_group_id
  protocol = "tcp"
}
# creates IAM role for ebs-csi-controller and assigns AmazonEBSCSIDriverPolicy policy to it
module "ebs-csi-controller_role_cluster-b" {
  source = "terraform-aws-modules/iam/aws//modules/iam-eks-role"
  role_name = "ebs-csi-controller-role-cluster-b"
  cluster_service_accounts = {
    (module.eks-cluster-b.cluster_name) = ["kube-system:ebs-csi-controller-sa"]
  }
  role_policy_arns = {
    "arn" = "arn:aws:iam::aws:policy/service-role/AmazonEBSCSIDriverPolicy"
  }
  depends_on = [
     module.eks-cluster-b.cluster_name # implicit dependancy doesn't work for some reason ...
  ]
}
